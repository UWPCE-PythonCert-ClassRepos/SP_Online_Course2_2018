...
def lookup():
    while True:
        donor = input('Please enter the Full Name of the donor')
        if donor == 'list':
            print(donors.names)
            continue
        else:
    ...
    phone = r.lindex(donor, 0)
    zipcode = r.lindex(donor, 1)
    email = r.lindex(donor, 2)
    print(f'Donor, {donor}, is located in {zipcode} and can be'
