        ...
        for job in jobs:
            with database.transaction():
                new_job = Job.create(
                    ...
                    job_department = job[DEPT_NUMBER],
...
def populate_deptdata():
    ...
    try:
        ...
        for job in Departement:
            logger.info(f'{job.department_name} has number {job.department_number} and is run by {job.department_manager}')
...
def pprint_db():
    ...
    try:
        ...
        for person in query:
            try:
                dept = Department.get(Department.department_number == person.job.job_department)
                print(f'Person {person.person_name} had job {person.job.job_name}')
                print(f'and reported to {dept.department_manager} for {person.job.duration} days')
                print(f'in the {dept.department_name} department')
