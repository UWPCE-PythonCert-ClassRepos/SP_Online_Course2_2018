...
class Decider():
    ...
    def __init__(self, target_height, margin):
        ...
        self.margin = margin
        self.height_min = target_height - margin * target_height
        self.height_max = target_height + margin * target_height
    ...
    def decide(self, current_height, current_action, actions):
        ...
        """
        new_action = 'PUMP_OFF'  # fallback
        if current_action == 0:
            if current_height < self.height_min:
                new_action = 'PUMP_IN'
            elif current_height >= self.height_max:
                new_action = 'PUMP_OUT'
            else:
                new_action = 'PUMP_OFF'

        if current_action == 1:
            if current_height >= self.height_max:
                new_action = 'PUMP_OFF'
            else:
                new_action = 'PUMP_IN'
