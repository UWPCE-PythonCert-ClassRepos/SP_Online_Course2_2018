    ...
    def project(factor, min_donation, max_donation):
        ...
        with login_database.login_mongodb_cloud() as client:
            ...
            if answer == "Yes":
                # donor.update(
                #     { 'donations'},
                #     { '$set' : { 'donations.$[element]': 222} }, # {'$multiply' : ["donations.$[element]", factor]} 
                #     { 'arrayFilters': [ { 'element' : { '$gt' : _min, '$lt' : _max} } ], '$upsert' : 'False' }
                # )
                select_upd = { 'donations' : {
                                                '$elemMatch' : { '$gt' : _min, '$lt' : _max}
                                              }
                     }
                new_values = { '$mul': { 'donations.$[element]':  factor} # multiplies all elements in array
                              }
