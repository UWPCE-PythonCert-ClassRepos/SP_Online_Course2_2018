...
class Decider(object):
    ...
    def __init__(self, target_height, margin):
        ...
        and below the target height for which the pump should not turn on.
        Ex: .05 represents a 5% margin above and below the target_height.
        """
        self.target_height = target_height
        self.margin = margin

    def pump_off_switcher(self, current_height, actions):
        """
        PUMP_OFF dispatcher
        """
    ...
    def pump_in_switcher(self, current_height, actions):
        """
        PUMP_IN dispatcher
        """
    ...
    def pump_out_switcher(self, current_height, actions):
        """
        PUMP_OUT dispatcher
        """
        if current_height < self.target_height:
    ...
    def decide(self, current_height, current_action, actions):
        ...
        pump_state_switcher = {
            actions['PUMP_OFF']: self.pump_off_switcher,
            actions['PUMP_IN']: self.pump_in_switcher,
            actions['PUMP_OUT']: self.pump_out_switcher,
        }
        method = pump_state_switcher.get(current_action)
        return method(current_height, actions)
