import meta_mailroom

d1 = meta_mailroom.Donor("kim schultz", [10, 100, 1000, 10000])
d2 = meta_mailroom.Donor("ann schultz", [9, 99, 999])
d3 = meta_mailroom.Donor("bill schultz", [222, 2222])
donors = meta_mailroom.DonorCollection([d1, d2, d3])

#PREVIOUS DONOR TESTS
def test_name():
...
def test_new_donor():
    d4 = meta_mailroom.Donor("sammy schultz", [222, 2222])
    assert d4.name == "sammy schultz"

# NEW JSON TESTS
def test_save_json():
    donors.save_json()
    assert len(d1.donations) == 4
    d1.append_donation(d1, d1, 420)
    assert len(d1.donations) == 5
    donors.load_json()
    assert len(d1.donations) == 4
