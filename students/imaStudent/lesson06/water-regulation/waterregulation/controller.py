...
class Controller(object):
    ...
    def tick(self):
        ...
        """
        sensor_measurement = self.sensor.measure()
        current_pump_state = self.pump.get_state()
        next_pump_state = self.decider.decide(sensor_measurement,
                                              current_pump_state, self.actions)
        status = self.pump.set_state(next_pump_state)
