...
class Decider():
    ...
    def __init__(self, target_height, margin):
        ...
        self.margin = margin
        self.low_range = target_height * (1 - margin)
        self.high_range = target_height * (1 + margin)

    def decide(self, current_height, current_action):
        """
        ...
        if current_action == 'PUMP_OFF':
            if current_height < self.low_range:
                action = 'PUMP_IN'
            elif current_height > self.high_range:
                action = 'PUMP_OUT'
            else:
            ...
            print("Current action not in actions dict:", current_action)
            action = False
