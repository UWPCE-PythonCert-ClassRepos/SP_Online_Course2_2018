...
class DonorData(object):
    ...
    def __init__(self):
        self.donor_data_ = self.load_json_data()

    @staticmethod
    def load_json_data():
        with open('donordata.txt') as json_file:
            donor_data = json.load(json_file)
            return donor_data

    def save_json_data(self):
        if self.donor_data_:
            with open('donordata.txt', 'w') as outfile:
                json.dump(self.donor_data_, outfile)
        else:
            exit()
