...
# Lesson_4 Activity 1 Json saved mailroom
...
class DonorChart:
    ...
    def create_report(self):
        ...
        report = []
        page_break()
        new_list = [sum(donor.donations) for donor in self.donors]
        col_lab = ["Donor Name", "Total Given", "Num Gifts", "Average Gift"]
        max_name = max([len(x) for x in self.donor_list])
        max_don = [max(donor.donations) for donor in self.donors]
        ...
        sorted_list = self.sort_by_total()
        for donor in sorted_list:
            ...
            format_item += "{}".format(max_donl+5) + ",.2f}"
            report.append(format_item.format(donor.full_name,
                                             donor.total_donation,
                                             num_gifts, avg_gift))
        report.append(f"Total raised = $ {self.total_raised:,.2f} "
                      f"from {self.total_donations:d} donations")
        print('\n'.join(report))
        return ('\n'.join(report))

    def save_json(self, filename=donors):
        """ save donor list to .json file """
        save_file = js._to_json_compat(self)
        with open(f"{filename}.json", 'w+') as file:
            json.dump(save_file, file)
        print('Saved file to JSON')

    def load_json(self, filename=donors):
        """load donor list from .json file"""
        with open(f"{filename}.json", 'r') as file:
            load_file = json.load(file)
            donor_list = self.from_json_dict(load_file)
...
def challenge_proj(factor=2, min_donation=0, max_donation=9999999999):
    """ask for a range of donations and factor
    to multiply all donations in that range then calculate
    challenge total"""
    ...
    if min_donation != -1 and max_donation == -1:
        ...
        for donor in mult_dc.donors:
            ...
            str_form2 += "donations less than ${} by {}: "
            str_form2 += "${:.2f} "*len(donor.donations)
            str_form2 += "\n{}'s new total: ${:.2f}"
    ...
    print(f"New Total from all contributions: "
          f"${mult_dc.total_raised+dc.total_raised:.2f}")
...
def load():
    """get a filename and run the load function"""
    while True:
        try:
            filename = input("Please type the filename without extension: ")
            cwd = os.getcwd()
            dir_path = cwd + "\\" + filename
            global dc
            dc = dc.load_json(dir_path)
            dc.create_report()
            break
...
def save():
    """get a filename and run the save function"""
    while True:
        try:
            filename = input("Please type the filename without extension: ")
            cwd = os.getcwd()
            dir_path = cwd + "\\" + filename
            dc.save_json(dir_path)
            break
        except FileNotFoundError:
            os.mkdir(dir_path)
            dc.save_json(dir_path)
...
options = range(1, 8)
test_dump = ""
menus = (send_thanks, print_report, send_letters,
         challenge_proj, menu_quit, save, load)
