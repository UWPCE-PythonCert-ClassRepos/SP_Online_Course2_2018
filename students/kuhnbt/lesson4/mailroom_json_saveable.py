"""json-saveable mailroom program"""
import os
import datetime
import sys
...
class DonorCollection(js.JsonSaveable):
    ...
    def save_json(self):
        try:
            ...
            self.json_representation = self.to_json_compat()
            with open(filename, 'w') as f:
                f.write(json.dumps(self.json_representation))
...
def load_from_json(filename):
    ...
    return DonorCollection.from_json_dict(txt)
...
def start_program(dc):
    ...
    selection_dict = {'1': dc.send_thank_you, '2': dc.create_report,
                      ...
                      '5': dc.save_json, '6': load_from_json}
    while True:
...
if __name__ == '__main__':
    donors = input('Load from file? Y/N')
    if donors.lower() == 'n':
        donors = DonorCollection()
        start_program(donors)
    else:
        filename =  input('Please enter filename of donors file')
        try:
            donors = load_from_json(filename)
