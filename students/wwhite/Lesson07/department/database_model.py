...
class Department(BaseModel):
    """
        ...
        departments each job falls under
    """

    logger.info('This is the Department Class')
    logger.info('Department Number')
    dep_number = CharField(primary_key=True, max_length=4, constraints=[
        Check('upper(substr(dep_number, 1, 1)BETWEEN "A" AND "Z")')
    ])
    logger.info('Department Name')
    dep_name = CharField(max_length=30)
    logger.info('Department Manager')
    dep_manager = CharField(null=False)
...
class Job(BaseModel):
    ...
    logger.info('Which department did the job fall under')
    job_dep = ForeignKeyField(Department, related_name='within_dep', null=False)
