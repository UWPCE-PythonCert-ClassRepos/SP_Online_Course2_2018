...
class Decider(object):
    ...
    def decide(self, current_height, current_action, actions):
        ...
        :return: The new action for the pump:
            one of actions['PUMP_IN'], actions['PUMP_OUT'],
                actions['PUMP_OFF']
        """
        ...
        if current_action == actions['PUMP_OFF']:
            if current_height < lower_margin:
                pump_status = actions['PUMP_IN']
            elif current_height > upper_margin:
                pump_status = actions['PUMP_OUT']
            else:
                pump_status = current_action
        elif current_action == actions['PUMP_IN']:
            if current_height > upper_margin:
                pump_status = actions['PUMP_OFF']
            else:
                pump_status = current_action
        else:
            if current_height < lower_margin:
