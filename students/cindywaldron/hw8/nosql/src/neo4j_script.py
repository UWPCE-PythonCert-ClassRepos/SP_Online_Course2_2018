...
def run_example():
    ...
    with driver.session() as session:
        ...
        for color in ['RED', 'BLUE', 'GREEN', 'BLACK', 'YELLOW', 'ORANGE', 'BROWN']:
            cyph_color = "CREATE (n:Color {whichcolor:'%s'})" % (color)
            session.run(cyph_color)
        ...
        log.info('Associate Bob with favoriate colors')
        ...
        log.info("Find Bob's favorite colors")
        cyph = """
          MATCH (bob {first_name:'Bob', last_name:'Jones'})
                -[:COLOR]->(bobColors)
          RETURN bobColors
        ...
        log.info('Associate Nancy with favoriate colors')
        for color in ['GREEN', 'BLACK', 'YELLOW']:
            cypher = """
        ...
        log.info("Find Nancy's favorite colors")
        cyph = """
          MATCH (bob {first_name:'Nancy', last_name:'Cooper'})
                -[:COLOR]->(bobColors)
          RETURN bobColors
          """
        result = session.run(cyph)
        print("Nancy's favorite colors are:")
        for rec in result:
            for color in rec.values():
                print(color['color'])