...
def run_example(furniture_items):
    ...
    with login_database.login_mongodb_cloud() as client:
        ...
        query = {"product": {"$eq": "couch"}, "color":{"$eq": "Blue"}}
        results = furniture.find_one(query)
        ...
        log.info(
            'Step 7a: Find all red products and print')

        cursor = furniture.find({'color': {'$eq': 'Red'}}).sort('monthly_rental_cost', 1)
        print('Results of search')
        ...
        log.info(
            'Step 7b: Find all couches and print')

        cursor = furniture.find({'product': {'$eq': 'couch'}}).sort('monthly_rental_cost', 1)
        print('Results of search')
        ...
        log.info(
            'Step 8 (Assignment): Find all red couches and print')
        cursor = furniture.find({'product': {'$eq': 'couch'}}).sort('monthly_rental_cost', 1)
        print('Results of search')
        for doc in cursor:
            print(f"Color: {doc['color']} Product_Type: {doc['product']}")
