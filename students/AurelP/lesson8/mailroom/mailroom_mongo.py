...
def add_donor_q():
    ...
    if x in don_lst:
        ...
        #y = str(input("Enter a donor email: "))
        #z = str(input("Enter a donor phone: "))
        add_donor(x)
...
def write():
    ...
    for d in donors.keys():
        file_name = d + ".txt"
        # print(d)
        # print(float(donors[d][-1]))
        if donors[d][0] is not None:
            with open(file_name, 'w') as f:
                f.write(txt.format(d, float(donors[d][-1])))
                print("Generated letter for {:s}!\n".format(d))

def all_donations():
    donor_dict=donors_dict(
    #list_of_donations()
    )
...
def donors_data():
    print("Option not available for mailroom mongo db")
    # donor_dict=donors_data_dict(
    # #list_of_donations()
    # )
    # for item, value in donor_dict.items():
    #     print('Donor: {} contact info: '.format(item[:-5]), value)
