...
class Donors_All:
    ...
    def _donation_summary(self):
        ...
        for donor in self._donors:
            sort_rep.append([donor.name, donor.total_donations, donor.len_donations, donor.avg_donations])
    ...
    def add_donation(self,x,d):
        #print ("add_donation")
        if x not in self._get_donor_names(): self.add_donor(Donor(x,[]))
    ...
    def report(self):
        ...
        """
        print("\n")
        print('Donor Name'+' '*16+'|'+' '*9+'Total Given'+' |'+' '*7+'Num Gifts'+
                          ' |'+' '*8+'Average Gift')
        print('-'*26+'|'+'-'*21+'|'+'-'*17+'|'+'-'*20+'|')
        for donor in self._donation_summary():
            #print("{:23}${:12.2f}{:10}   ${:12.2f}".format())
