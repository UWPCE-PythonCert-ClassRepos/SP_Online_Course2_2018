...
class Decider(object):
    ...
    def decide(self, current_height, current_action, actions):
        ...
        """
        upper_height = self.target_height * (1 + self.margin)
        lower_height = self.target_height * (1 - self.margin)

        if current_action == actions['PUMP_OFF']:  # 0
            if current_height < lower_height:
                action = 'PUMP_IN'
            elif current_height > upper_height:
                action = 'PUMP_OUT'
            else:
                action = 'PUMP_OFF'

        if current_action == actions['PUMP_IN']:  # 1
            if current_height > self.target_height:
                action = 'PUMP_OFF'
            else:
                action = 'PUMP_IN'

        if current_action == actions['PUMP_OUT']:  # -1
            if current_height < self.target_height:
                action = 'PUMP_OFF'
            else:
                action = 'PUMP_OUT'
