...
def run_nosql_ex(furniture_items):
    """
        mongodb data lesson 08
    """
    ...
    with login_database.login_mongodb_cloud() as client:
        ...
        db = client['dev']
        furniture = db['furniture']
        log.info('Data saved to monogodb')
        furniture.insert_many(furniture_items)

        query = {'color': 'Red'}
        results = furniture.find(query)

        log.info('Print products colored red')
        print('Red products')
        for i in results:
            pprint.pprint(i)

        log.info('Find all the couches')
        query = {'product_type': 'Couch'}
        results_1 = furniture.find(query)
        for j in results_1:
            pprint.pprint(j)
