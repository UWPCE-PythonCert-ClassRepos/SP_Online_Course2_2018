...
def run_example(furniture_items):
    ...
    with login_database.login_mongodb_cloud() as client:
        ...
        log.info('Step 5a: Find the blue couch(es)')
        cursor = furniture.find({'product_type': 'Couch', 'color': 'blue'})
        for doc in cursor:
            print(f"Product name: {doc['product_type']}, Description: {doc['description']}")
        ...
        log.info('Step 7a: Just get the red items.')
        cursor = furniture.find({'color': 'red'})
        for doc in cursor:
            print(f"Product name: {doc['product_type']}, Description: {doc['description']}")

        log.info('Step 7b: Now just get the couches.')
