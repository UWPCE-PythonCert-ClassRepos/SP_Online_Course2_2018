...
class Decider(object):
    ...
    def decide(self, current_height, current_action, actions):
        ...
        upper = self.target_height + (self.target_height * self.margin)
        lower = self.target_height - (self.target_height * self.margin)

        new = current_action

        if current_action == actions['PUMP_OFF']:
            if current_height > upper:
                new = actions['PUMP_OUT']
            elif current_height < lower:
                new = actions['PUMP_IN']
            else:
                new = actions['PUMP_OFF']
