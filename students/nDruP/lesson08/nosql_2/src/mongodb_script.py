...
def run_example(furniture_items):
    ...
    """
    input('MongoDB Example. Press enter to continue...........')
    ...
    with login_database.login_mongodb_cloud() as client:
        ...
        furniture.remove({"product_type": {"$eq": "couch"}, 'color': 'Blue'})
        ...
        for doc in cursor:
            print(f"Cost: {doc['monthly_rental_cost']} "
                  f"product name: {doc['product_type']} "
                  f"Stock: {doc['in_stock_quantity']}")

        log.info('Assignment: Change the Mongodb program '
                 "to store and retrieve using these new values.")
        log.info("Next, write a mongodb query to retrieve and print "
                 "just the red products, and the just the couches.")

        cursor = furniture.find({'color': 'Red', 'product_type': 'Couch'})
        for doc in cursor:
            print(f"{doc['color']} {doc['description']} {doc['product_type']}")
        
        log.info('Step 8: Delete the collection so we can start over')
        db.drop_collection('furniture')
        pprint.pprint(furniture.find())
