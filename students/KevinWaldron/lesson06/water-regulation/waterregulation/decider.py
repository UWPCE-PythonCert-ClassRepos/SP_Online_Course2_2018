...
class Decider():
    ...
    def decide(self, current_height, current_action, actions):
        ...
        """
        max_region = self.tgt + self.tgt * self.margin
        min_region = self.tgt - self.tgt * self.margin
        ...
        if current_action == Pump.PUMP_IN and current_height <= self.tgt:
            return actions['PUMP_IN']
        if current_action == Pump.PUMP_OUT and current_height >= self.tgt:
            return actions['PUMP_OUT']
