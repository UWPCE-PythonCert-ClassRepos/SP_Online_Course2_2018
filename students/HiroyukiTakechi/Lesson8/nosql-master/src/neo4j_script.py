"""
    neo4j example. Lesson 8 Assignment 1.
"""
...
def run_example():
    ...
    with driver.session() as session:
        ...
        log.info('Additional Step 1 (Hiro): Add some new people to the database.')
        for first, last in [('Hiro', 'Takechi'),
                            ('Maiko', 'Nakamura'),
                            ]:
            cyph = "CREATE (n:Person {first_name:'%s', last_name: '%s'})" % (
                first, last)
        ...
        log.info('Additional Step 2 (Hiro): Add some colors.')
        for color in ['blue', 'pink', 'red', 'white', 'black', 'brown', 'gold', 'silver', 'yellow', 'gray', 'orange']:
            cyph = "CREATE (c:color {name: '%s'})" % (
                color)
            session.run(cyph)

        log.info('Additional Step 2 (Hiro): Create associations between people and their favorite colors.')
        for first, last in [('Bob', 'Jones'),
                            ('Nancy', 'Cooper'),
                            ('Alice', 'Cooper'),
                            ('Fred', 'Barnes'),
