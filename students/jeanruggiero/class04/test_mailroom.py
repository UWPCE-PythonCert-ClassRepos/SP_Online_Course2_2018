...
import mailroom
from mailroom import DonorDict
...
def test_dd_save():
    donors2.save('test_out.json')
    with open('test_out.json','r') as f:
        donors_json_in = f.read()
    assert donors_json_in == donors_json_out

# Test FP sub-functions
def test_thank_donor():
    assert mailroom.thank_donor(donors, 'name four', 77) == \
        mailroom.thank(Donor('name four', [77]), 77)

def test_thank():
    assert mailroom.thank(don, 6) == "Dear Name Zero,\n\n" + \
...
def test_filename():
    ...
    assert mailroom.filename(don) == '_'.join(['Name_Zero', str(d.month),
            str(d.day), str(d.year)])+'.txt'

def test_challenge():
    assert mailroom.challenge(donors, 2) == \
        DonorDict([
            Donor('name one', [2,4]),
            Donor('name two', [6,8,10]),
            Donor('name four', [154])
            ])
    assert mailroom.challenge(donors, 2, min_donation=4) == \
        DonorDict([
            Donor('name one', [1,2]),
            Donor('name two', [3,8,10]),
            Donor('name four', [154])
            ])
    assert mailroom.challenge(donors, 3, max_donation=2) == \
        DonorDict([
            Donor('name one', [3,6]),
            Donor('name two', [3,4,5]),
            Donor('name four', [77])
            ])
    assert mailroom.challenge(donors, 4, min_donation=2, max_donation=5) == \
        DonorDict([
            Donor('name one', [1,8]),
...
def test_load_file():
    assert mailroom.load_file('test_in.json') == donors2
    with pytest.raises(FileNotFoundError):
        mailroom.load_file('blahrandomname')
    with pytest.raises(mailroom.json.JSONDecodeError):
        mailroom.load_file('mailroom.py')
    with pytest.raises(mailroom.JSONConversionError):
        mailroom.load_file('corrupted.json')
