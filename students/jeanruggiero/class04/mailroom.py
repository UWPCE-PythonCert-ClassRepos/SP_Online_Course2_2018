...
class JSONConversionError(Exception): pass
...
def load():
    """Load donors from a user input .json filename."""
    global donors
    if donors is not None:
        while True:
            c = input('This will overwrite current donor data. Continue? ' + \
                '[Y/N]: ')
            if c.lower() == 'n':
                return
            elif c.lower() == 'y':
                break
    while True:
        fname = input("Enter the name of the file to load from: ")
        if fname.lower() == 'return':
            return
        try:
            donors = load_file(fname)
            break
        except FileNotFoundError:
            print("File doesn't exist, please try again")
            continue
        except (json.JSONDecodeError, JSONConversionError):
            print("File could not be converted, please try again.")
            continue

def save():
    """Save currently loaded donors information to a .json file."""
    while True:
        fname = input('Enter the name of a file to write to: ')
        if os.path.isfile(fname):
            c = input('File already exists. Overwrite? [Y/N]: ')
            if c.lower() != 'y':
                continue
        donors.save(fname)
        break
...
def load_file(fname):
    """
    Helper function to open and create a DonorDict object out of a json file.
    """
    with open(fname) as f:
        donors = from_json(f)
    # Error handling in case the file is corrupted and objects aren't
    # loaded properly.
    if not isinstance(donors, DonorDict):
    ...
    while True:
        try:
            ...
            action = input('''
                ...
                4: Match Donations
                5: Project Donations
                6: Load Donors from .json File
                7: Save Donors to .json File
                8: Quit\n
                ''')
            actions.get(action)()
        except ExitScript:
