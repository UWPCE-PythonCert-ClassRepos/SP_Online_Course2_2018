...
class Decider():
    ...
    def decide(self, current_height, current_action, actions):
        ...
        if current_action not in actions.values():
            raise KeyError(str(current_action) + ' is not a valid action.')

        if current_action == actions['PUMP_OFF'] and current_height < \
                self.target_height - self.margin:
            new_action = 'PUMP_IN'
        elif current_action == actions['PUMP_OFF'] and current_height > \
                self.target_height + self.margin:
            new_action = 'PUMP_OUT'
        elif current_action == actions['PUMP_OFF'] and \
                (current_height >= self.target_height - self.margin or
