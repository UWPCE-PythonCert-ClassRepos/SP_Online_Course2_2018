...
def run_exercise():
    ...
    For an extra assignment, write a program that reads form one format and converts to another.


    """
    log.info("Step 1: Read from json input file")
    people_list = []
    with open('../data/westeros.json', 'r') as json_in:
        data = json.load(json_in)
        for row in data:
            people_list.append(row)
            print(row)

    log.info("Step 2: Write to json output file")
    with open('../data/westeros_out.json', 'w') as json_out:
        json.dump(people_list,json_out)

    log.info("Step 3: Convert and write to csv output file")
    with open('../data/westeros_out.csv', 'w') as csv_out:
        writer = csv.writer(csv_out)
        log.info("Write header row")
        header_line = [header for header in people_list[0]]
        print(header_line)
        writer.writerow(header_line)
        log.info("Write people rows")
        for person in people_list[1:]:
            person_line = [person[key] for key in person]
            print(person_line)
            writer.writerow(person_line)
