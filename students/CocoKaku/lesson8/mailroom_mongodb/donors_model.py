...
class Donations():
    ...
    def add_donation(self, name, amount):
        donation = {'name': name, 'amount': amount}
        self.mycol.save(donation)
    ...
    def challenge(self, factor, min_donation=0, max_donation=1e10):
        cursor = self.mycol.aggregate(
            [
                {"$match": {"amount": {"$gte": min_donation, "$lte": max_donation}}},
    ...
    def delete_donation(self, name, amount):
        found = self.mycol.delete_one({'name': name, 'amount': amount})
