"""
    Database implementation of mailroom (Donation)
    Using Peewee ORM, sqlite and Python
    Here we define the schema/model
"""
...
class Donation(BaseModel):
    ...
    def summary_report(cls):
        ...
        for item in query:
            report += f"{item.name:20s}   ${item.sum:12,.2f} {item.count:3d}               ${item.sum/item.count:11,.2f}\n"
        return report
    ...
    def challenge(cls, factor, min_donation=None, max_donation=None):
        ...
        report = ""
        for d in query:
            report += f"   {d.name}: ${factor*float(d.sum):,.2f} = {factor} * ${float(d.sum):,.2f}\n"
