...
class Donor_DB:
    ...
    def save(self):
        save_data = self.to_json_compat()
        save_data['donors'] = [[d.name, d.donations] for d in save_data['donors']]
        with open('Donor_DB.json', 'w') as file:
            file.write(str(save_data))
            
    def load(self):
        with open('Donor_DB.json', 'r') as file:
            new_db = file.read()
            new_db = ast.literal_eval(new_db)
        
        [self.add_donor(Donor(name, donation)) for name, donation in new_db['donors'] if name not in self.display_names()]
...
if __name__ == '__main__':
    ...
    while True:
        prompt = input("Enter:\n Send a Thank You (1)\n Create a Report (2)\n Send Letter to Everyone (3)\n Save the donor database (4)\n Load the donor database (5)\n or quit (6) > ")
