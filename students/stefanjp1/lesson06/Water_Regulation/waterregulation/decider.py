...
class Decider(object):
    ...
    def decide(self, current_height, current_action, actions):
        ...
        over_max_height = current_height > self.target_height*(1+self.margin)
        over_target_height = current_height > self.target_height

        under_min_height = current_height < self.target_height*(1-self.margin)
        under_target_height = current_height < self.target_height

        within_margin_level = not over_max_height and not under_min_height

        if current_action == actions['PUMP_OFF']:
            if under_min_height:
                next_pump_state = 'PUMP_IN'
            elif over_max_height:
                next_pump_state = 'PUMP_OUT'
            elif within_margin_level:
                next_pump_state = 'PUMP_OFF'

        elif current_action == actions['PUMP_OUT']:
            if under_target_height:
                next_pump_state = 'PUMP_OFF'
            else:
                next_pump_state = 'PUMP_OUT'

        elif current_action == actions['PUMP_IN']:
            if over_target_height:
                next_pump_state = 'PUMP_OFF'
            else:
                next_pump_state = 'PUMP_IN'

        return actions[next_pump_state]
