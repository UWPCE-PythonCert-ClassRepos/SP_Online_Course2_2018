...
"""
Lesson 04 - 
Update Mailroom Script to be able to save and load from JSON
$ chmod +x mailroom_json.py
"""
...
class DonorFunctions:
    ...
    def save_json(self):
        """ JSON Donor list"""
        donorsave = jsd._to_json_compat(self)
        with open('donorList_updated.json', 'w+') as fileout:
            json.dump(donorsave, fileout)
        print('Donors Saved to File')

    def load_json(self):
        """Loads JSON file to program"""
        global donorslist
        with open('donorList.json', 'r') as infile:
            donor_dict = json.load(infile)
            donorslist = self.from_json_dict(donor_dict)
            self.mailroom_donors = donorslist
...
def load_new_donors():
    global mailroom_donors
    mailroom_donors = mailroom_donors.load_json()
    mailroom_donors.print_donors_and_donations()
def save_json():
    mailroom_donors.save_json()

def print_report():
    mailroom_donors.print_report()

def send_letters_everyone():
    mailroom_donors.send_letters_everyone()

def print_letters_to_everyone():
    mailroom_donors.print_letters_to_everyone()

def send_single_thank_you():
    mailroom_donors.send_single_thank_you()

def print_donors_names():
    mailroom_donors.print_donors_names()

def print_donors_and_donations():
    mailroom_donors.print_donors_and_donations()

def print_donors_and_donation_totals():
    mailroom_donors.print_donors_and_donation_totals()

#Donors to start the program with
...
mailroom_donors = DonorFunctions([donor1, donor2])
...
MAIN_MENU = {
    ...
    "7": {'menu_prompt': 'Save Donors to Donor File', 'menu_dispatch': save_json},
    "8": {'menu_prompt': 'Load Donors List from File', 'menu_dispatch': load_new_donors},
...
SINGLE_PRINT_SUB_MENU = {
    ...
    "3": {'menu_prompt': 'Print Donors and All Donations', 'menu_dispatch': print_donors_and_donations},
    "4": {'menu_prompt': 'Print Donors and Donation Totals', 'menu_dispatch': print_donors_and_donation_totals},
