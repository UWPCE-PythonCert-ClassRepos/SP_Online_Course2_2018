...
class Decider():
    ...
    def decide(self, current_height, current_action, actions):
        ...
        low_margin_height = self.target_height * (1 - self.margin)
        up_margin_height = self.target_height * (1 + self.margin)

        # Situation 1
        if current_action == "PUMP_OFF" and current_height < low_margin_height:
            result = actions["PUMP_IN"]

        # Situation 2
        if current_action == "PUMP_OFF" and current_height > up_margin_height:
            result = actions["PUMP_OUT"]

        # Situation 3
        if (current_action == "PUMP_OFF" and low_margin_height
                <= current_height <= up_margin_height):
            result = actions["PUMP_OFF"]

        # Situation 4
        if current_action == "PUMP_IN":
            result = actions[current_action]
