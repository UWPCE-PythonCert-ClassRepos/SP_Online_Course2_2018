...
def run_example(furniture_items):
    ...
    def run_my_pickle():
        bakery = pickle_data.get_bakery_data()
        """
        Write and read with pickle
        """
        log.info("\n\n====")
        log.info('Step 1: Demonstrate persistence with pickle')
        log.info('Write a pickle file with the bakery data')

        pickle.dump(bakery, open('../data/data1.pkl', 'wb'))

        log.info('Step 2: Now read it back from the pickle file')
        read_data = pickle.load(open('../data/data1.pkl', 'rb'))
        log.info('Step 3: Show that the write and read were successful')
        assert read_data == bakery
        log.info("and print the data")
        pprint.pprint(read_data)

        log.info("The  read and write is the same: " + str(read_data ==
                                                            bakery))
