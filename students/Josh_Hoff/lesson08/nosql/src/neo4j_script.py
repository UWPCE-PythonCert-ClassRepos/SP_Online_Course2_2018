    ...
    with driver.session() as session:
        ...
        for first, last in new_people:
            cyph = "CREATE (n:Person {first_name:'%s', last_name: '%s'})" % (
                first, last)
            session.run(cyph)
        color_list = ['Red', 'Blue', 'Purple', 'Yellow', 'Green']
        for new_color in ['Red', 'Blue', 'Purple', 'Yellow']:
            cypher = """
              MATCH (p1:Person {first_name:'Jordan', last_name:'Sanders'})
              CREATE (p1)-[favorite:FAVORITE]->(c:Color {color:'%s'})
              RETURN p1
            """ % (new_color)
            session.run(cypher)
            
        for new_color in ['Red', 'Blue']:
            cypher = """
              MATCH (p1:Person {first_name:'Brandon', last_name:'Blackburn'})
              CREATE (p1)-[favorite:FAVORITE]->(c:Color {color:'%s'})
              RETURN p1
            """ % (new_color)
            session.run(cypher)
            
        for new_color in ['Purple', 'Green']:
            cypher = """
              MATCH (p1:Person {first_name:'Mike', last_name:'Hoff'})
              CREATE (p1)-[favorite:FAVORITE]->(c:Color {color:'%s'})
              RETURN p1
            """ % (new_color)
            session.run(cypher)

#keeping for reference - pulling a single person's favorite colors
#        cyph = """
#          MATCH (jordan {first_name:'Jordan', last_name:'Sanders'})
#                -[:FAVORITE]->(colors)
#          RETURN colors
#          """
#        result = session.run(cyph)
#        print("\nJordan's favorite colors are:")
#        for rec in result:
...
#                print(ind['color'])
        ...
        for first, last in new_people:
            ...
            print(f"\n{first} {last}'s favorite colors are:")
            for rec in result:
                for ind in rec.values():
                    print(ind['color'])
        ...
        for color in color_list:
            cypher = """
              MATCH ((p1)-[FAVORITE]->(c:Color {color: '%s'}))
              RETURN p1
              """ % (color)
            result = session.run(cypher)
            
            print(f'\nThe people that like the color {color} are:')
            for rec in result:
                for ind in rec.values():
                    print(ind['first_name'], ind['last_name'])