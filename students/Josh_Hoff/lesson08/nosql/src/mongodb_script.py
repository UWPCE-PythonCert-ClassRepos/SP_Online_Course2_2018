...
def run_example(furniture_items):
    ...
    with login_database.login_mongodb_cloud() as client:
        ...
        log.info('PRINTING RED PRODUCTS')
        cursor = furniture.find({'color': 'Red'})
        for doc in cursor:
            print(f"Product type: {doc['product type']} Color: {doc['color']} Stock: {doc['in_stock_quantity']}")
            
        log.info('PRINTING COUCHES')
        cursor = furniture.find({'product type': 'Couch'})
        for doc in cursor:
            print(f"Product type: {doc['product type']} Color: {doc['color']} Stock: {doc['in_stock_quantity']}")
