...
with open('donors.json') as f:
    donor_data_raw = json.load(f)

donor_data=defaultdict(list)
for i in donor_data_raw.keys():
        donor_data[i] = donor_data_raw[i]
...
main_dispatch ={'1':sub1_menu,'2':Collection(donor_data).creat_a_report,'3':Collection(donor_data).send_letters_to_everyone,
                '4':Collection(donor_data).save_report, '5':sys.exit}
