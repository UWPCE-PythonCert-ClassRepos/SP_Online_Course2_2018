...
from donation_tracker_dec import Donor, Donorlist
...
def test_save_list():
    dl = Donorlist()
    dl.add_donor('Donor1')
    dl.add_donation('Donor1', 15)
    output = StringIO()
    dl.save_list(output)
    saved_file = output.getvalue()
    assert saved_file == '{\n    "__obj_type": "Donorlist",\n    "_donor_objects": {\n        "Donor1": {\n            "__obj_type": "Donor",\n            "name": "Donor1",\n            "_donations": [\n                15.0\n            ]\n        }\n    }\n}'

def test_load_file():
        in_file = StringIO('{\n    "__obj_type": "Donorlist",\n    "_donor_objects": {\n        "Donor1": {\n            "__obj_type": "Donor",\n            "name": "Donor1",\n            "_donations": [\n                15.0\n            ]\n        }\n    }\n}')
        dl1 = Donorlist()
        dl1.add_donor('Donor1')
        dl1.add_donation('Donor1', 15)
        dl2 = Donorlist.load_file(in_file)
        assert dl2 == dl1
